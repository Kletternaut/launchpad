...
function LaunchpadLogoMark() {
  return <img src="/favicon.svg" width={26} height={26} alt="Launchpad" className="shrink-0" />;
}
  ...
  return (
    <div className="desktop bg-bg text-text">
      ...
      >
        <div className="w-full px-3 sm:px-4 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2" style={{ textDecoration: 'none' }}>
            <LaunchpadLogoMark />
            <span className="text-lg font-extrabold tracking-tight gradient-text select-none">
              Launchpad
            </span>
          </Link>
          <nav className="flex items-center gap-1">
            ...
            {onNewLink && (
              <Button type="button" variant="ghost" size="sm" onClick={onNewLink} aria-label="New link" leadingIcon={<NewLinkIcon />}>
      ...
      {editLayoutActive && onEditLayout && (
        <div className="sticky z-30 w-full backdrop-blur-md" style={{ top: 'var(--header-height)', background: `${theme.surface}ee`, borderBottom: `1px solid ${theme.border}` }}>
          <div className="w-full px-3 sm:px-4 py-1.5 flex justify-end">
            <Button type="button" variant="primary" size="sm" onClick={onEditLayout} leadingIcon={<svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75 9.75 18 19.5 6" /></svg>}>Save layout</Button>
          </div>
        </div>
      )}
